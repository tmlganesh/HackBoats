    ...
    return (
        <header className="fixed top-0 left-0 right-0 z-50 px-4 sm:px-6 py-3 sm:py-4">
            <div className="max-w-7xl mx-auto">
                <div className="flex items-center justify-between backdrop-blur-md bg-[#0B1F3A]/90 border border-white/10 rounded-full px-4 sm:px-6 py-2.5 sm:py-3">
                    {/* Logo */}
                        ...
                        <NavigationMenuList className="flex gap-1">
                            {navItems.map((item) => (
                                <NavigationMenuItem key={item.name}>
                                    <Link
                                        to={item.href}
                                        className={`group inline-flex h-10 w-max items-center justify-center rounded-full px-4 py-2 text-sm font-medium transition-all duration-300 ease-out text-white/70 hover:text-white hover:bg-white/5 focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-500/50 ${location.pathname === item.href ? 'text-cyan-400' : ''}`}
                                    >
                                        {item.name}
                                    </Link>
                                </NavigationMenuItem>
                    ...
                    <div className="hidden lg:flex items-center gap-4">
                        <Link
                            to="/media"
                            className="inline-flex items-center justify-center px-5 py-2 text-sm font-medium text-white hover:text-cyan-400 transition-colors duration-300"
                        >
                            Media
                        </Link>
                        <Link
                            to="/#contact"
                            className="inline-flex items-center justify-center px-5 py-2 text-sm font-medium text-black bg-white rounded-full hover:bg-gray-200 transition-colors duration-300"
                        >
                            Get Started
                        </Link>
                    </div>
